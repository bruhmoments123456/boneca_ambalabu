...
st.title("funny")
tabroblox,tabfacebook,tabmessanger,tabtest,tabhelp,tabteststeps=st.tabs(["roblox","facebook","messanger","test","help","steps"])
with tabroblox:
...
with tabtest:
    ...
    age=st.number_input("YoUr AgE:",min_value=1.0,max_value=100.0,value=18.0,step=0.0)
    iq=st.number_input("YoUr Iq:",min_value=0.0,max_value=1000.0,value=100.0,step=0.1)
    if st.button("Age"):
        st.success(f"Ok age:{age:.0f}")
    if st.button("IQ"):
